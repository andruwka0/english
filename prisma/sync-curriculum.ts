...
const lessons: LessonDefinition[] = [
  {
    ...
    tasks: [
      task("1. Сложение", "В переменных a и b лежат числа. Создай total, сложи a и b и выведи total.", "a = 7\nb = 3\n\n# Создай total и выведи его\n", "assert total == a + b\nassert output.strip() == str(total)"),
      task("2. Деление", "В переменных apples и friends лежат числа. Создай part - результат обычного деления apples на friends. Выведи part.", "apples = 9\nfriends = 2\n\n# Используй обычное деление /\n", "assert part == apples / friends\nassert output.strip() == str(part)"),
      task("3. Умножение", "В переменных rows и stickers лежат числа. Создай total, умножь rows на stickers и выведи total.", "rows = 4\nstickers = 5\n\n# Посчитай total\n", "assert total == rows * stickers\nassert output.strip() == str(total)"),
      ...
      task("6. Копилка", "В переменной money лежит количество монет. Если money больше 10, выведи Можно купить игру, иначе выведи Копим дальше.", "money = 12\n\n# Напиши if и else\n", "expected = 'Можно купить игру' if money > 10 else 'Копим дальше'\nassert output.strip() == expected"),
    ],
  },
  {
